  ...
  <div>
    <form class="card auth-card" @submit.prevent="submitHandler">
      ...
      <div class="input-field">
        ...
        >
          {{"PasswordMustBe" | localize}} {{$v.password.$params.minLength.min}} {{"Characters" | localize}} {{password.length}}</small
        >
      </div>
        <div class="input-field">
...
export default {
  ...
  methods: {
    async submitHandler () {
      if (!this.$v.agree.checked) {
        this.$message(messages['reigsterCheck']);
      }
      if (this.$v.$invalid) {      
        this.$v.$touch();
        return;
